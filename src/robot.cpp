...
namespace robot
{
	...
	auto PegInHole::executeRT() -> int
	{
		...
		{
			//Tool
			double assem_pos[6]{ 0.600, 0.039101, 0.291316, PI / 2, -PI / 2, PI / 2 };
			double assem_angle[6]{ 0 };
			double assem_rm[9]{ 0 };
			double assem_pm[16]{ 0 };
			double stop_count = 2000;
			...
			if (count() % 1000 == 0)
			{
				mout() << "angle: " << current_sa_angle[0] << '\t' << current_sa_angle[1] << '\t' << current_sa_angle[2] << '\t'
					<< current_sa_angle[3] << '\t' << current_sa_angle[4] << '\t' << current_sa_angle[5] << std::endl;
			}


			if (motorsPositionCheck(current_sa_angle, assem_angle, 6) && !imp_->stop)
			{
				mout() << "end angle: " << current_sa_angle[0] << '\t' << current_sa_angle[1] << '\t' << current_sa_angle[2] << '\t'
					<< current_sa_angle[3] << '\t' << current_sa_angle[4] << '\t' << current_sa_angle[5] << std::endl;

				mout() << "Assembly Start Pos Reached" << std::endl;

				imp_->current_count = count();
				imp_->stop = true;
			...
			if (imp_->stop)
			{
				if (imp_->current_count + stop_count == count())
				{
					imp_->phase1 = true;
